...
const ShapeGrid = ({
  ...
  useEffect(() => {
    ...
    // Pre-bake one glowing hex sprite. shadowBlur runs exactly once here, then
    // every per-frame "glow" is just drawImage which the GPU composites cheaply.
    const glowSprite = (() => {
      if (!isHex || !glowColor || glowBlur <= 0) return null;
      const pad = Math.ceil(glowBlur * 2 + 4);
      const w = Math.ceil(squareSize * 2 + pad * 2);
      const h = Math.ceil(squareSize * 2 + pad * 2);
      ...
      for (let i = 0; i < 6; i++) {
        ...
        if (i === 0) octx.moveTo(vx, vy);
        else octx.lineTo(vx, vy);
      }
      octx.closePath();
      // Two passes thicken the bloom without ever calling shadowBlur per-frame.
      octx.stroke();
      octx.stroke();
    ...
    const drawGrid = () => {
      ...
      const hasGlow = !!glowColor && glowBlur > 0;
      ...
      if (isHex) {
        ...
        // Pass 1 — base strokes for every hex, no glow.
        ...
        // Pass 2 — accent glow via pre-baked sprite (no per-frame shadowBlur).
        if (hasGlow && glowSprite) {
          const { canvas: spr, halfW, halfH } = glowSprite;
          for (let col = -2; col < cols; col++) {
            for (let row = -2; row < rows; row++) {
              const cx = col * hexHoriz + offsetX;
              const cy =
              ...
              const wave = Math.sin((cx + cy) * 0.012 - t * 0.9);
              if (wave < 0.25) continue;

              ctx.globalAlpha = (wave - 0.25) / 0.75;
              ctx.drawImage(spr, cx - halfW, cy - halfH);
            }
          }
          ctx.globalAlpha = 1;
        }

        // Pass 3 — hover fills on top, no glow.
        for (let col = -2; col < cols; col++) {
          for (let row = -2; row < rows; row++) {
    ...
    const updateAnimation = (now: number) => {
      ...
      const stepScale = Math.min(dt / (1000 / 60), 3);
      const effectiveSpeed = Math.max(speed, 0.1) * stepScale;
